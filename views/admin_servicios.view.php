    ...
    <div class="contenedor-sm">
        <ul class="cuenta__listado">  
            <?php foreach($servicios as $servicio):?>          
                <li class="cuenta">
                    <p class="cuenta__texto"><?php echo $servicio['nombre']; ?></p>
                    <div class="cuenta__opciones">
                        <!-- <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
                            <input type="hidden" name="id_prod_ser" id = "id_prod_ser" value="<?php echo $servicio['id_prod_ser']; ?>">
                            <button  id="estado" name="estado" class="cuenta__opciones--boton cuenta__opciones--<?php echo ($servicio['estado'] == 1) ? 'habilitado' : 'pendiente'; ?>">
                                <?php echo ($servicio['estado'] == 1) ? 'Habilitado' : 'Pendiente'; ?>
                            </button>                    
                        </form> -->
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
